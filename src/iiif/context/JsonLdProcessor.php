...
class JsonLdProcessor
{
    ...
    public static function isDictionary($dictionary) {
        ...
        foreach ($dictionary as $key => $value) {
            ...
            if ($value!=null && !is_scalar($value) && !is_array($value)) return false;
        }
        return true;
    }
    ...
    protected function createTermDefinition(JsonLdContext $activeContext, $localContext, $term, &$defined) {
        ...
        if (array_key_exists(Keywords::CONTAINER, $value)) {
            $allowedInContainer = [Keywords::GRAPH, Keywords::ID, Keywords::INDEX, Keywords::LANGUAGE, Keywords::LIST, Keywords::SET, Keywords::TYPE];
            ...
            if (is_string($container) && array_search($container, $allowedInContainer)!==false) {
                ...
                if (count($container) == 1 && array_search($container[0], $allowedInContainer)!==false) {
                    ...
                    $valid = (count($container) == 2 && (array_search(Keywords::ID, $container)!==false || array_search(Keywords::INDEX, $container)!==false)) ||
                    (count($container) == 3 && (array_search(Keywords::ID, $container)!==false || array_search(Keywords::INDEX, $container)!==false) && array_search(Keywords::SET, $container)!==false);
                } elseif (array_search(Keywords::SET, $container)!==false) {
                    $valid = true;
                    $allowedWithSet = [Keywords::INDEX, Keywords::ID, Keywords::TYPE, Keywords::LANGUAGE, Keywords::SET];
